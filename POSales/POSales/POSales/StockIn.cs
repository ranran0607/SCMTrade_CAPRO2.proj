    ...
    public partial class StockIn : Form
    {
        ...
        private void btnLoad_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                cm = new SqlCommand("SELECT * FROM vwStockIn WHERE CAST(sdate as date) BETWEEN '" + dtFrom.Value.ToString("yyyyMMdd") + "' AND '" + dtTo.Value.ToString("yyyyMMdd") + "' AND status LIKE 'Done'", cn);
                dr = cm.ExecuteReader();
                while (dr.Read())
                {
                    i++;
                    ...
                    dgvInStockHistory.Rows.Add(i, dr[0].ToString(), dr[1].ToString(), dr[2].ToString(), dr[3].ToString(), dr[4].ToString(), DateTime.Parse(dr[5].ToString()).ToString("yyyyMMdd"), dr[6].ToString(), dr["supplier"].ToString());
                }
                dr.Close();
                cn.Close();
            }
