...
namespace POSales
{
    public partial class Record : Form
    {
        ...
        public void LoadTopSelling()
        {
            ...
            {
                cm = new SqlCommand("SELECT TOP 10 pcode, pdesc, isnull(sum(qty),0) AS qty, ISNULL(SUM(total),0) AS total FROM vwTopSelling WHERE sdate BETWEEN '" + dtFromTopSell.Value.ToString("yyyy-MM-dd") + "' AND '" + dtToTopSell.Value.ToString("yyyy-MM-dd") + "' AND status LIKE 'Sold' GROUP BY pcode, pdesc ORDER BY qty DESC", cn);
            }
            else if (cbTopSell.Text == "Sort By Total Amount")
            {
                cm = new SqlCommand("SELECT TOP 10 pcode, pdesc, isnull(sum(qty),0) AS qty, ISNULL(SUM(total),0) AS total FROM vwTopSelling WHERE sdate BETWEEN '" + dtFromTopSell.Value.ToString("yyyy-MM-dd") + "' AND '" + dtToTopSell.Value.ToString("yyyy-MM-dd") + "' AND status LIKE 'Sold' GROUP BY pcode, pdesc ORDER BY total DESC", cn);
            }
            dr = cm.ExecuteReader();
            while(dr.Read())
            {
        ...
        public void LoadSoldItems()
        {
            try
            {
                ...
                cn.Open();
                cm = new SqlCommand("SELECT ISNULL(SUM(total),0) FROM tbCart WHERE status LIKE 'Sold' AND sdate BETWEEN '" + dtFromSoldItems.Value.ToString("yyyy-MM-dd") + "' AND '" + dtToSoldItems.Value.ToString("yyyy-MM-dd") + "'", cn);
        ...
        public void LoadCancelItems()
        {
            ...
            cn.Open();
            cm = new SqlCommand("SELECT * FROM vwCancelItems WHERE sdate BETWEEN '" + dtFromCancel.Value.ToString("yyyy-MM-dd") + "' AND '" + dtToCancel.Value.ToString("yyyy-MM-dd") + "'", cn);
            dr = cm.ExecuteReader();
            while(dr.Read())
            {
                i++;
                ...
                dgvCancel.Rows.Add(i, dr[0].ToString(), dr[1].ToString(), dr[2].ToString(), dr[3].ToString(), dr[4].ToString(), dr[5].ToString(), DateTime.Parse(dr[6].ToString()).ToString("yyyy-MM-dd"), dr[7].ToString(), dr[8].ToString(), dr[9].ToString(), dr[10].ToString());
            }
            dr.Close();
            cn.Close();
        }
        ...
        public void LoadStockInHist()
        {
            ...
            cn.Open();
            cm = new SqlCommand("SELECT * FROM vwStockIn WHERE cast(sdate AS date) BETWEEN '" + dtFromStockIn.Value.ToString("yyyy-MM-dd") + "' AND '" + dtToStockIn.Value.ToString("yyyy-MM-dd") + "' AND status LIKE 'Done'", cn);
            dr = cm.ExecuteReader();
            while (dr.Read())
            {
                i++;
                dgvStockIn.Rows.Add(i, dr[0].ToString(), dr[1].ToString(), dr[2].ToString(), dr[3].ToString(), dr[4].ToString(), DateTime.Parse(dr[5].ToString()).ToString("yyyyMMdd"), dr[6].ToString(), dr[7].ToString(), dr[8].ToString());
            }
            dr.Close();
            cn.Close();
        }
        ...
        private void btnPrintSoldItems_Click(object sender, EventArgs e)
        {
            ...
            string param = "From : " + dtFromSoldItems.Value.ToString("yyyy-MM-dd") + " To : " + dtToSoldItems.Value.ToString("yyyy-MM-dd");
        ...
        {
            POSReport report = new POSReport();
            string param = "From : " + dtFromTopSell.Value.ToString("yyyy-MM-dd") + " To : " + dtToTopSell.Value.ToString("yyyy-MM-dd");
            if (cbTopSell.Text == "Sort By Qty")
            {
                report.LoadTopSelling("SELECT TOP 10 pcode, pdesc, isnull(sum(qty),0) AS qty, ISNULL(SUM(total),0) AS total FROM vwTopSelling WHERE sdate BETWEEN '" + dtFromTopSell.Value.ToString("yyyy-MM-dd") + "' AND '" + dtToTopSell.Value.ToString("yyyy-MM-dd") + "' AND status LIKE 'Sold' GROUP BY pcode, pdesc ORDER BY qty DESC", param, "TOP SELLING ITEMS SORT BY QTY");
            }
            else if (cbTopSell.Text == "Sort By Total Amount")
            {
        ...
        {
            POSReport report = new POSReport();
            string param = "From : " + dtFromCancel.Value.ToString("yyyy-MM-dd") + " To : " + dtToCancel.Value.ToString("yyyy-MM-dd");
            report.LoadCancelledOrder("SELECT * FROM vwCancelItems WHERE sdate BETWEEN '" + dtFromCancel.Value.ToString("yyyy-MM-dd") + "' AND '" + dtToCancel.Value.ToString("yyyy-MM-dd") + "'", param);
            report.ShowDialog();
        }
        ...
        {
            POSReport report = new POSReport();
            string param = "From : " + dtFromStockIn.Value.ToString("yyyy-MM-dd") + " To : " + dtToStockIn.Value.ToString("yyyy-MM-dd");
            report.LoadStockInHist("SELECT * FROM vwStockIn WHERE cast(sdate AS date) BETWEEN '" + dtFromStockIn.Value.ToString("yyyy-MM-dd") + "' AND '" + dtToStockIn.Value.ToString("yyyy-MM-dd") + "' AND status LIKE 'Done'", param);
            report.ShowDialog();
        }
    }
}
